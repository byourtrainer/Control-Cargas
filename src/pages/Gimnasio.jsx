...
function SeccionSesiones({ perfil }) {
  ...
  async function cargarPlantillas() {
    ...
    const { data } = await supabase
      ...
      .select('*, gimnasio_plantilla_items(id)')
      .order('nombre')
    setPlantillas(data || [])
    setCargando(false)
  }
  ...
  async function guardarPlantilla() {
    ...
    if (plantillaActivaId === 'nueva') {
      ...
      await supabase.from('gimnasio_plantilla_items').delete().eq('plantilla_id', idPlantilla)
    }

    const filas = items.map((it, i) => ({
      plantilla_id: idPlantilla, ejercicio_id: it.ejercicio_id, orden: i,
    ...
    const { error: errorItems } = await supabase.from('gimnasio_plantilla_items').insert(filas)
            ...
            {plantillas.map((p) => (
              <button key={p.id} className="gimnasio-tarjeta" onClick={() => abrirPlantilla(p)}>
                <div>
                  ...
                  <span className="texto-dim">{p.gimnasio_plantilla_items?.length || 0} ejercicio(s)</span>
                </div>
  ...
  return (
    <div className="gimnasio-seccion">
      ...
      <div className="sesiones-cuerpo-simple">
        <button type="button" className="btn-principal" onClick={() => setModalAbierto(true)}>
          🔍 Explorar biblioteca y añadir ejercicios
        </button>

        <section className="sesiones-items-panel">
          <h3>Ejercicios de la sesión ({items.length})</h3>
          {items.length === 0 ? (
            <p className="texto-dim">Añade ejercicios con el botón de arriba.</p>
          ) : (
              ...
              {items.map((it, i) => (
                <div className="sesiones-item-card" key={i}>
                  ...
                  <div className="sesiones-item-cuerpo">
                    ...
                    <div className="sesiones-item-variables">
                    ...
                    <input
                      className="sesiones-item-notas-input" placeholder="Notas para esta sesión (opcional)"
